package viewpager.dangngocduc.android.library;
...
/**
 * Created by DANG NGOC DUC on 6/17/2016.
 */
import android.content.Context;
...
public class ViewPagerVertical extends ViewGroup {
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        final float pageDelta = (float) Math.abs(dy) / (getHeight() + mPageMargin);
    ...
    private void recomputeScrollPosition(int height, int oldHeight, int margin, int oldMargin) {
        final int heightheightMargin = height + margin;
        if (oldHeight > 0) {
            final int oldScrollPos = getScrollY();
            final int oldhhm = oldHeight + oldMargin;
            final int oldScrollItem = oldScrollPos / oldhhm;
            final float scrollOffset = (float) (oldScrollPos % oldhhm) / oldhhm;
            final int scrollPos = (int) ((oldScrollItem + scrollOffset) * heightheightMargin);
            scrollTo(getScrollX(),scrollPos);
            if (!mScroller.isFinished()) {
                // We now return to your regularly scheduled scroll, already in progress.
                final int newDuration = mScroller.getDuration() - mScroller.timePassed();
                mScroller.startScroll(0,scrollPos, 0, mCurItem * heightheightMargin, newDuration);
            }
        } else {
            int scrollPos = mCurItem * heightheightMargin;
            if (scrollPos != getScrollY()) {
                completeScroll();
                scrollTo(scrollPos, getScrollX());
            }
        }
    }
    ...
    public void computeScroll() {
        ...
        if (!mScroller.isFinished()) {
            if (mScroller.computeScrollOffset()) {
                ...
                if (mOnPageChangeListener != null) {
                    final int heightHeightMargin =getHeight() + mPageMargin;
                    final int position = y / heightHeightMargin;
                    final int offsetPixels = y % heightHeightMargin;
                    final float offset = (float) offsetPixels / heightHeightMargin;
        ...
        switch (action) {
            case MotionEvent.ACTION_MOVE: {
                ...
                */
                ...
                final boolean atEdge = (dy > 0 && scrollY == 0) || (dy < 0 && mAdapter != null &&
                        scrollY >= (mAdapter.getCount() - 1) * getHeight() - 1);
        ...
        switch (action & MotionEventCompat.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsBeingDragged) {
                    ...
                    final int height = getHeight();
                    final int heightMargin = height + mPageMargin;

                    final int lastItemIndex = mAdapter.getCount() - 1;
                    final float topBound = Math.max(0,
                            (mCurItem - 1) * heightMargin);
                    final float bottomBound =
                            Math.min(mCurItem + 1, lastItemIndex) * heightMargin;
                    if (scrollY < topBound) {
                        if (topBound == 0) {
                            float over = -scrollY;
                            needsInvalidate = mLeftEdge.onPull(over / height);
                        }
                        scrollY = topBound;

                    } else if (scrollY > bottomBound) {
                        if (bottomBound == lastItemIndex * heightMargin) {
                            float over = scrollY - bottomBound;
                            needsInvalidate = mRightEdge.onPull(over / height);
                        }
                        scrollY = bottomBound;
                    }
                    ...
                    if (mOnPageChangeListener != null) {
                        final int position = (int) scrollY / heightMargin;
                        final int positionOffsetPixels = (int) scrollY % heightMargin;
                        final float positionOffset = (float) positionOffsetPixels / heightMargin;
            ...
            case MotionEvent.ACTION_UP:
                if (mIsBeingDragged) {
                    ...
                    final int heightMargin = getHeight() + mPageMargin;
                    final int scrollY = getScrollY();
                    final int currentPage = scrollY / heightMargin;
...
//                mLeftEdge.setSize( getWidth(),getHeight());
...
//            }
    ...
    public void fakeDragBy(float yOffset) {
        ...
        final float topBound = Math.max(0, (mCurItem - 1) * heightMargin);

        final float bottomBound = Math.min(mCurItem + 1, mAdapter.getCount() - 1) * heightMargin;
        if (scrollY < topBound) {
            scrollY = topBound;
        } else if (scrollY > bottomBound) {
            scrollY = bottomBound;
    ...
    private void onSecondaryPointerUp(MotionEvent ev) {
        ...
        if (pointerId == mActivePointerId) {
            ...
            mLastMotionY= MotionEventCompat.getX(ev, newPointerIndex);
    ...
    public boolean arrowScroll(int direction) {
        ...
        if (nextFocused != null && nextFocused != currentFocused) {
            if (direction == View.FOCUS_UP) {
                ...
                if (currentFocused != null && nextFocused.getBottom() <= currentFocused.getBottom()) {
                    handled = pageBottom();
                } else {
                    handled = nextFocused.requestFocus();
                }
